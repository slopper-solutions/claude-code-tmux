...
- `bin/claude-*` — deprecated thin forwarders that print a warning and exec the matching `agent-*`. Kept for backwards compatibility; will be removed in a future release.
- `bin/agent-tmux-launch`, `bin/agent-random-name` — internal utilities. The first is invoked by the systemd unit to bootstrap the foundational agent (Claude Code in Phase 1). The second is a slug generator used as the default name for spawned agents.
- `agent-tmux.service` — systemd user unit; invokes `bin/agent-tmux-launch`, which produces the `[<host>][claude] main` window.
- `config.env` — reference file documenting `REMOTE_CONTROL` and `SKIP_PERMISSIONS`. The actual runtime config is written to `~/.config/agents/config.env` interactively on first install.
